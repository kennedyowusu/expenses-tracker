    ...
    <>
      {profile?.accounts?.length > 0 ? (
        <section className='py-20'>
          <h1 className='uppercase text-center text-indigo-600 font-bold'>Account Summary For {profile?.accounts?.length}
            {
              profile?.accounts?.length > 1 ? ' Accounts' : ' Account'
            }
          </h1>
          <div className='container mx-auto px-4 mt-4'>
            <div className='py-4 flex flex-wrap items-center text-center rounded-lg border'>
              ...
              <div className='py-4 w-full md:w-1/2 lg:w-1/4 border-b md:border-b-0 lg:border-r'>
                ...
                <span className='text-3xl lg:text-4xl font-bold text-indigo-600'>${totalBalance}</span>
              </div>
              <div className='py-4 w-full md:w-1/2 lg:w-1/4'>
                <h4 className='mb-2 text-gray-500'>Total Transactions</h4>
                <span className='text-3xl lg:text-4xl font-bold'>{transactions.length}</span>
              </div>
            </div>
          </div>
        </section>
